...
string t,s= "<tag1 value = \"HelloWorld\">\n</tag1>" ;
...
for(x:s)
   if(m.find(x[0])==m.end())
      for(int i=1 ;i<strlen(s) ;i=i+2)
         m[ s[0] ].insert(std::make_pair(s[i], s[i+1])); // tag1 v1 x v2 y
   ...
   string temp = "";
    ...
    for (int i = 0; i < tags.length(); i++) {
        ...
        if (tags.at(i) == '<') {
            ...
            string temp = tags.substr(i + 1, fmin(tags.find('>', i + 1), 
...
tags.find(' ', i + 1)) - i - 1);
    ...
    string result = tags.substr(tags.find('"', index) + 1, 


tags.find('"', tags.find('"', index) + 1) - tags.find('"', index) - 1);
